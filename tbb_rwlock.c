...
void tbb_start_write(tbb_rwlock_t *l) {
    ...
    while (1) {
        ...
        if (l->counter & TBB_CRITICAL) {
             ...
             * RINC may get lost. */
            __sync_fetch_and_or((uint16_t *)l, TBB_WPENDING);
        ...
        if (__sync_bool_compare_and_swap((uint16_t *)l, tmp, TBB_WRITER))
            return;
    }
}
...
void tbb_end_write(tbb_rwlock_t *l) {
    ...
    __sync_fetch_and_and((uint16_t *)l, ~(TBB_WPENDING | TBB_WRITER));
}
...
void tbb_start_read(tbb_rwlock_t *l) {
    ...
    while (1) {
        ...
        if (! (l->counter & (TBB_WPENDING | TBB_WRITER))) {
            lval = __sync_fetch_and_add((uint16_t *)l, TBB_RINC);
            ...
            __sync_fetch_and_add((uint16_t *)l, -TBB_RINC);
        }
    }
}
